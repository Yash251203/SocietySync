...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setUser(true); // Set user to true after successful login or registration
      navigate('/dashboard');
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      .catch(() => {
        ...
        setUser(false); // Set user to false if authentication fails
      });
    } else {
      setUser(false); // Set user to false if no token is found
    }
  }, [navigate]);
